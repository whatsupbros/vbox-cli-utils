...
# Created: 08.08.2018
# Purpose: Restores the snapshot with the specified name for the specified virtual machine
#
# Usage: ./07_snapshot_restore.sh [virtual_machine_name/UUID] [snapshot_name]
...
# and promts the user to specify snapshot name or UUID to restore
else
    echo -e "\nAvailable snapshots:"
    ...
    echo -e "\n"
    read -p "Enter name or UUID of snapshot to RESTORE: " snapshot;
fi
